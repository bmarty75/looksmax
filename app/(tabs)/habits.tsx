...
export default function Habits() {
  ...
  const saveChecked = async (c: Record<string, boolean>) => {
    setChecked(c);
    await storage.set(`lm_checked_${todayKey()}`, c);
    const s = await storage.get("lm_stats", { totalChecked: 0, waterCount: 0 });
    await storage.set("lm_stats", s);
  };
  ...
  const addHabit = async () => {
    ...
    const newH = { id: `h_${Date.now()}`, ...form };
    await saveHabits([...habits, newH]);
    setForm({ label: "", icon: "🎯", category: "custom", color: COLORS[0] });
    setShowForm(false);
  };

  const deleteHabit = (id: string) => {
    Alert.alert("Supprimer", "Tu veux vraiment supprimer cette habitude ?", [
    ...
    <ScrollView style={styles.root} contentContainerStyle={{ paddingBottom: 30 }}>
      ...
      <View style={{ marginBottom: 16 }}>
        <Text style={styles.progressText}>
          <Text style={{ color: "#C9A96E", fontWeight: "700" }}>{completedToday}</Text>/{habits.length} complétées
        </Text>
      ...
      {showForm && (
        <View style={styles.formCard}>
          <View style={styles.formRow}>
            ...
            <TextInput style={[styles.input, { flex: 1 }]} placeholder="Nom de l'habitude..."
              placeholderTextColor="#333" value={form.label}
              ...
              {COLORS.map((c) => (
                <TouchableOpacity key={c} onPress={() => setForm((f) => ({ ...f, color: c }))}
                  style={[styles.colorDot, { backgroundColor: c },
                    form.color === c && { borderWidth: 2, borderColor: "#fff", transform: [{ scale: 1.3 }] }]} />
              ))}
            </View>
          </View>
      ...
      <Modal visible={pickerOpen} transparent animationType="slide">
        <TouchableOpacity style={styles.modalOverlay} onPress={() => setPickerOpen(false)}>
          <View style={styles.pickerModal}>
            ...
            <FlatList data={ICONS} numColumns={5} keyExtractor={(i) => i}
              renderItem={({ item }) => (
                <TouchableOpacity style={styles.pickerCell}
        ...
        <View key={cat} style={{ marginBottom: 16 }}>
          ...
          {habits.filter((h) => h.category === cat).map((h) => (
            <TouchableOpacity key={h.id}
              onPress={() => toggle(h.id)}
              onLongPress={() => deleteHabit(h.id)}
              style={[styles.habitCard, {
                borderColor: checked[h.id] ? h.color : "#222",
                backgroundColor: checked[h.id] ? `${h.color}20` : "#111",
              }]}>
              ...
              <View style={[styles.checkbox, {
                ...
                borderColor: checked[h.id] ? h.color : "#333",
              }]}>
      ...
      <Text style={styles.hint}>💡 Appui long pour supprimer une habitude</Text>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  header: { paddingTop: 60, paddingBottom: 16, borderBottomWidth: 1, borderBottomColor: "#161616", marginBottom: 16 },
  ...
  progressTrack: { height: 3, backgroundColor: "#1A1A1A", borderRadius: 2, overflow: "hidden" },
  ...
  formCard: { backgroundColor: "#0F0F0F", borderWidth: 1, borderColor: "#1A1A1A", borderRadius: 14, padding: 16, marginBottom: 16, gap: 12 },
  formRow: { flexDirection: "row", gap: 10, alignItems: "center" },
  formLabel: { fontSize: 11, color: "#555", fontWeight: "700", letterSpacing: 1, minWidth: 70 },
  iconPick: { width: 48, height: 48, backgroundColor: "#161616", borderWidth: 1, borderColor: "#2A2A2A", borderRadius: 10, alignItems: "center", justifyContent: "center" },
  catChip: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, borderWidth: 1, borderColor: "#2A2A2A", marginRight: 8 },
  catChipActive: { borderColor: "#C9A96E44", backgroundColor: "#C9A96E11" },
  catChipText: { fontSize: 11, color: "#555", fontWeight: "700" },
  colorRow: { flexDirection: "row", gap: 8, flexWrap: "wrap" },
  colorDot: { width: 22, height: 22, borderRadius: 11 },
  input: { backgroundColor: "#080808", borderWidth: 1, borderColor: "#222", borderRadius: 8, color: "#EEE", padding: 10, fontSize: 14 },
  ...
  pickerModal: { backgroundColor: "#0F0F0F", borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, maxHeight: 400 },
  pickerTitle: { color: "#888", fontSize: 11, letterSpacing: 3, fontWeight: "700", textAlign: "center", marginBottom: 16 },
  pickerCell: { flex: 1, aspectRatio: 1, alignItems: "center", justifyContent: "center" },
  catLabel: { fontSize: 9, letterSpacing: 3, color: "#333", fontWeight: "700", marginBottom: 8, paddingLeft: 4 },
  habitCard: { flexDirection: "row", alignItems: "center", gap: 14, borderWidth: 1, borderRadius: 12, padding: 14, marginBottom: 8 },
  habitIcon: { width: 38, height: 38, borderRadius: 10, alignItems: "center", justifyContent: "center" },
  habitLabel: { flex: 1, fontSize: 15, color: "#CCC", fontWeight: "500" },
  checkbox: { width: 24, height: 24, borderRadius: 6, borderWidth: 1.5, alignItems: "center", justifyContent: "center" },
  hint: { textAlign: "center", color: "#333", fontSize: 11, marginTop: 8, marginBottom: 16 },
});