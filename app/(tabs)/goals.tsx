...
export default function Goals() {
  ...
  useEffect(() => {
    storage.get("lm_goals", DEFAULT_GOALS).then(setGoals);
  }, []);
  ...
  const addGoal = async () => {
    ...
    const newG = { id: `g_${Date.now()}`, ...form, target: parseInt(form.target) || 30, progress: 0 };
    const s = await storage.get("lm_stats", { goalsCreated: 0 });
    await storage.set("lm_stats", { ...s, goalsCreated: s.goalsCreated + 1 });
    await saveGoals([...goals, newG]);
    setForm({ label: "", target: "30", unit: "j", icon: "🎯", color: "#B07ECC" });
    setShowForm(false);
  };
    ...
    <ScrollView style={styles.root} contentContainerStyle={{ paddingBottom: 30 }}>
      ...
      {showForm && (
        <View style={styles.formCard}>
          ...
          <TextInput style={styles.input} placeholder="Nom de l'objectif..."
            placeholderTextColor="#333" value={form.label}
            onChangeText={(t) => setForm((f) => ({ ...f, label: t }))} />

          <View style={styles.formRow}>
            <Text style={styles.formLabel}>Cible</Text>
            <TextInput style={[styles.input, { width: 80 }]} keyboardType="numeric"
              value={form.target} onChangeText={(t) => setForm((f) => ({ ...f, target: t }))}
              placeholderTextColor="#333" />
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              {UNITS.map((u) => (
                <TouchableOpacity key={u} onPress={() => setForm((f) => ({ ...f, unit: u }))}
                  style={[styles.unitChip, form.unit === u && styles.unitChipActive]}>
                  <Text style={[styles.unitChipText, form.unit === u && { color: "#C9A96E" }]}>{u}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
          </View>
      ...
      {goals.length === 0 && (
        <View style={styles.emptyState}>
          <Text style={styles.emptyText}>Aucun objectif.{"\n"}Crée ton premier ! 🎯</Text>
        </View>
      )}
      ...
      {goals.map((g) => {
        ...
        const done = pct >= 100;
        return (
          <View key={g.id} style={[styles.goalCard, { borderColor: done ? g.color : "#1A1A1A" }]}>
            <View style={styles.goalTop}>
              <View style={styles.goalIconWrap}>
                <Text style={{ fontSize: 22 }}>{g.icon}</Text>
              </View>
              <View style={{ flex: 1 }}>
                <Text style={styles.goalLabel}>{g.label} {done ? "✅" : ""}</Text>
                <Text style={styles.goalSub}>{g.progress} / {g.target} {g.unit}</Text>
              </View>
            ...
            <View style={styles.goalActions}>
              <TouchableOpacity style={styles.goalBtn} onPress={() => updateProgress(g.id, -1)}>
                <Text style={styles.goalBtnText}>−</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.goalBtn, { borderColor: g.color, flex: 2 }]}
                onPress={() => updateProgress(g.id, 1)}>
                <Text style={[styles.goalBtnText, { color: g.color }]}>+1 {g.unit}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.goalBtn, { borderColor: "#E07B5A33" }]}
...
const styles = StyleSheet.create({
  ...
  addBtn: { backgroundColor: "#C9A96E22", borderWidth: 1, borderColor: "#C9A96E44", borderRadius: 8, paddingHorizontal: 14, paddingVertical: 6 },
  ...
  formLabel: { fontSize: 11, color: "#555", fontWeight: "700", letterSpacing: 1, minWidth: 60 },
  iconChip: { width: 44, height: 44, borderRadius: 10, backgroundColor: "#161616", borderWidth: 1, borderColor: "#2A2A2A", alignItems: "center", justifyContent: "center", marginRight: 8 },
  ...
  unitChip: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, borderWidth: 1, borderColor: "#2A2A2A", marginRight: 8 },
  ...
  goalCard: { backgroundColor: "#0F0F0F", borderWidth: 1, borderRadius: 14, padding: 18, marginBottom: 10 },
  goalTop: { flexDirection: "row", alignItems: "center", gap: 14, marginBottom: 14 },
  goalIconWrap: { width: 44, height: 44, backgroundColor: "#161616", borderRadius: 12, alignItems: "center", justifyContent: "center" },
  goalLabel: { fontSize: 15, fontWeight: "600", color: "#DDD", marginBottom: 3 },
  goalSub: { fontSize: 12, color: "#555" },
  goalPct: { fontSize: 18, fontWeight: "800" },
  goalBarTrack: { height: 4, backgroundColor: "#161616", borderRadius: 2, overflow: "hidden", marginBottom: 14 },
  goalBarFill: { height: "100%", borderRadius: 2 },
  goalActions: { flexDirection: "row", gap: 8 },
  goalBtn: { flex: 1, backgroundColor: "transparent", borderWidth: 1, borderColor: "#2A2A2A", borderRadius: 8, padding: 8, alignItems: "center" },
