...
function makeStyles(c: ThemeColors) {
  return StyleSheet.create({
    ...
    formLabel:      { fontSize: 11, color: c.textMuted, fontWeight: "700", letterSpacing: 1, minWidth: 60 },
    iconChip:       { width: 44, height: 44, borderRadius: 10, backgroundColor: c.surface, borderWidth: 1, borderColor: c.border2, alignItems: "center", justifyContent: "center", marginRight: 8 },
    ...
    unitChip:       { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, borderWidth: 1, borderColor: c.border2, marginRight: 8 },
    unitChipActive: { borderColor: "#C9A96E44", backgroundColor: "#C9A96E11" },
    unitChipText:   { fontSize: 11, color: c.textMuted, fontWeight: "700" },
    ...
    goalCard:       { backgroundColor: c.card, borderWidth: 1, borderRadius: 14, padding: 18, marginBottom: 10 },
    goalTop:        { flexDirection: "row", alignItems: "center", gap: 14, marginBottom: 14 },
    goalIconWrap:   { width: 44, height: 44, backgroundColor: c.surface, borderRadius: 12, alignItems: "center", justifyContent: "center" },
    goalLabel:      { fontSize: 15, fontWeight: "600", color: c.text, marginBottom: 3 },
    goalSub:        { fontSize: 12, color: c.textMuted },
    goalPct:        { fontSize: 18, fontWeight: "800" },
    goalBarTrack:   { height: 4, backgroundColor: c.surface, borderRadius: 2, overflow: "hidden", marginBottom: 14 },
    goalBarFill:    { height: "100%", borderRadius: 2 },
    goalActions:    { flexDirection: "row", gap: 8 },
    goalBtn:        { flex: 1, backgroundColor: "transparent", borderWidth: 1, borderColor: c.border2, borderRadius: 8, padding: 8, alignItems: "center" },
    goalBtnText:    { color: c.textSub, fontSize: 13, fontWeight: "700" },
  });
}
  ...
  return (
    <ScrollView style={styles.root} contentContainerStyle={{ paddingBottom: 30 }}>
        ...
        <View style={styles.formCard}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            {ICONS.slice(0, 10).map((ic) => (
              <TouchableOpacity
                key={ic}
                onPress={() => setForm((f: any) => ({ ...f, icon: ic }))}
          ...
          <TextInput
            ...
            value={form.label}
            onChangeText={(t: string) => setForm((f: any) => ({ ...f, label: t }))}
          />
          ...
          <View style={styles.formRow}>
            ...
            <TextInput
              ...
              value={form.target}
              onChangeText={(t: string) => setForm((f: any) => ({ ...f, target: t }))}
              placeholderTextColor={colors.textFaint}
            />
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              {UNITS.map((u) => (
                <TouchableOpacity
                  key={u}
                  onPress={() => setForm((f: any) => ({ ...f, unit: u }))}
      ...
      {goals.map((g: any) => {
        ...
        const done = pct >= 100;
        return (
          <View key={g.id} style={[styles.goalCard, { borderColor: done ? g.color : colors.border }]}>
